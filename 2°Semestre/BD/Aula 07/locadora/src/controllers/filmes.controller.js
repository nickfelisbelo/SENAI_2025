...
const faturamentoCategoria = async (req, res) => {
    try{
        const locacoes = await db.query("SELECT categoria, SUM(preco) AS faturamento_categoria FROM filmes INNER JOIN locacoes ON filmes.id = locacoes.filme_id GROUP BY categoria");
